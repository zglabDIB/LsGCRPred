...
feat_df=pd.read_csv(curr_dir+'/selected_transcripts/rs9510420.txt',sep='\t')
...
stdscaler=pickle.load(open(curr_dir+'/models/rs9510420/rs9510420_stdscaler.sav', 'rb'))
model=pickle.load(open(curr_dir+'/models/rs9510420/rs9510420_LR.sav', 'rb'))

X_train=stdscaler.transform(X_train)
pred_prob=np.round(model.predict_proba(X_train)[:,1]*100,2)

final_df=pd.DataFrame(columns=['sample_id','pred_proba'])
final_df['sample_id']=['rs9510420']
final_df['pred_proba']=pred_prob
final_df.to_csv(curr_dir+'/output_files/rs9510420_pred_ouput.txt',index=False,sep='\t')
