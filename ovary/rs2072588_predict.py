...
feat_df=pd.read_csv(curr_dir+'/selected_transcripts/rs2072588.txt',sep='\t')
...
stdscaler=pickle.load(open(curr_dir+'/models/rs2072588/rs2072588_stdscaler.sav', 'rb'))
model=pickle.load(open(curr_dir+'/models/rs2072588/rs2072588_LR.sav', 'rb'))

X_train=stdscaler.transform(X_train)
pred_prob=np.round(model.predict_proba(X_train)[:,1]*100,2)

final_df=pd.DataFrame(columns=['sample_id','pred_proba'])
final_df['sample_id']=['rs2072588']
final_df['pred_proba']=pred_prob
final_df.to_csv(curr_dir+'/output_files/rs2072588_pred_ouput.txt',index=False,sep='\t')
