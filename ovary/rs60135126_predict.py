...
feat_df=pd.read_csv(curr_dir+'/selected_transcripts/rs60135126.txt',sep='\t')
...
stdscaler=pickle.load(open(curr_dir+'/models/rs60135126/rs60135126_stdscaler.sav', 'rb'))
model=pickle.load(open(curr_dir+'/models/rs60135126/rs60135126_LR.sav', 'rb'))

X_train=stdscaler.transform(X_train)
pred_prob=np.round(model.predict_proba(X_train)[:,1]*100,2)

final_df=pd.DataFrame(columns=['sample_id','pred_proba'])
final_df['sample_id']=['rs60135126']
final_df['pred_proba']=pred_prob
final_df.to_csv(curr_dir+'/output_files/rs60135126_pred_ouput.txt',index=False,sep='\t')
