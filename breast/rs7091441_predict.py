...
feat_df=pd.read_csv(curr_dir+'/selected_transcripts/rs7091441.txt',sep='\t')
...
stdscaler=pickle.load(open(curr_dir+'/models/rs7091441/rs7091441_stdscaler.sav', 'rb'))
model=pickle.load(open(curr_dir+'/models/rs7091441/rs7091441_LR.sav', 'rb'))

X_train=stdscaler.transform(X_train)
pred_prob=np.round(model.predict_proba(X_train)[:,1]*100,2)

final_df=pd.DataFrame(columns=['sample_id','pred_proba'])
final_df['sample_id']=['rs7091441']
final_df['pred_proba']=pred_prob
final_df.to_csv(curr_dir+'/output_files/rs7091441_pred_ouput.txt',index=False,sep='\t')
